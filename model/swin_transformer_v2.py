...
class ConvMlp(nn.Module):
    ...
    def forward(self, x, H, W):
        B, L, C = x.shape
        assert L == H * W
        x = x.view(B, H, W, C).contiguous().permute(0, 3, 1, 2).contiguous()  # B C H W
        x = self.conv_proj(x)
        if self.proj_ln:
            x = self.proj_ln(x)
        x = x.permute(0, 2, 3, 1).contiguous()  # B H W C
        x = x.reshape(B, L, C).contiguous()
        x = self.mlp(x, H, W)
        return x
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
                 ...
                 rpe_output_type='normal', attn_type='cosine_mh', mlpfp32=False, pretrain_window_size=-1):
    ...
    def forward(self, x, mask=None):
        ...
        if self.q_bias is not None:
            qkv_bias = torch.cat((self.q_bias, torch.zeros_like(self.v_bias, requires_grad=False), self.v_bias)).contiguous()
        ...
        if self.attn_type == 'cosine_mh':
            ...
            q = q * self.scale
            attn = (q.float() @ k.float().transpose(-2, -1).contiguous())
        else:
            raise NotImplementedError()

        if self.relative_coords_table_type != "none":
            # relative_position_bias_table: 2*Wh-1 * 2*Ww-1, nH
            relative_position_bias_table = self.rpe_mlp(self.relative_coords_table).view(-1, self.num_heads).contiguous()
        else:
            relative_position_bias_table = self.relative_position_bias_table
        relative_position_bias = relative_position_bias_table[self.relative_position_index.view(-1).contiguous()].view(
        ...
        if mask is not None:
            nW = mask.shape[0]
            attn = attn.view(B_ // nW, nW, self.num_heads, N, N).contiguous() + mask.unsqueeze(1).unsqueeze(0)
        ...
        x = (attn @ v).transpose(1, 2).contiguous().reshape(B_, N, C).contiguous()
        if self.mlpfp32:
...
class SwinTransformerBlockPost(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
                 ...
                 relative_coords_table_type='norm8_log', rpe_hidden_dim=512,
                 rpe_output_type='normal', attn_type='cosine_mh', mlp_type='normal', mlpfp32=False,
    ...
    def forward(self, x, mask_matrix):
        ...
        x_windows = x_windows.view(-1, self.window_size * self.window_size, C).contiguous()  # nW*B, window_size*window_size, C
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        x = torch.cat([x0, x1, x2, x3], -1).contiguous()  # B H/2 W/2 4*C
...
class ConvPatchMerging(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        if self.postnorm:
            ...
            x = self.reduction(x).flatten(2).transpose(1, 2).contiguous()  # B H//2*W//2 2*C
            x = self.norm(x)
        else:
            x = self.norm(x)
            x = x.permute(0, 3, 1, 2).contiguous()  # B C H W
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.norm is not None:
            ...
            x = self.norm(x)
            x = x.transpose(1, 2).contiguous().view(-1, self.embed_dim, Wh, Ww).contiguous()
