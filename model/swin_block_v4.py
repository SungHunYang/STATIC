...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, v, mask=None):
        ...
        """
        B_, N, C = x.shape
        q = self.q(x).view(B_, N, self.num_heads, -1).contiguous().transpose(1, 2).contiguous()
        kv = self.kv(v).reshape(B_, N, 2, self.num_heads, C // self.num_heads).contiguous().permute(2, 0, 3, 1, 4).contiguous()
        ...
        logit_scale = torch.clamp(self.logit_scale,
                                  max=torch.log(torch.tensor(1. / 0.01)).to(self.logit_scale.get_device())).exp()
...
class SAMBLOCK(nn.Module):
    ...
    def forward(self, x, v, H, W):
        ...
        v_windows = v_windows.view(-1, self.window_size * self.window_size,
                                   v_windows.shape[-1]).contiguous()  # nW*B, window_size*window_size, C
...
class SAMv4(nn.Module):
    def __init__(self,
        ...
        self.resize = resize
        if self.resize:
            self.q_down = nn.Conv2d(embed_dim, embed_dim, 3, 2,padding=1)
            self.up = nn.ConvTranspose2d(embed_dim, embed_dim, 2, 2,padding=0)
